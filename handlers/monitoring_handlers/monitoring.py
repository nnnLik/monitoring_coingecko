...
async def monitoring(message: types.Message):
    buttons = ["📈 Check price", "✍🏻 Tracked coins", "📜 List of coins", "🔙 Back"]
...
async def check_price(message: types.Message):
    ...
    await message.answer(
        "Select the currency to which you want to see the coin price",
...
async def list_of_coins(message: types.Message):
    ...
    for coins_inf in coins_name_id:
        await message.answer(
            f"Coin Name: *{coins_inf[1]}*\nCoin ID: _{coins_inf[0]}_",
            parse_mode="Markdown",
        )


@dp.callback_query_handler(
    lambda command: command.data and command.data.startswith("CP_")
)
async def cp_add(callback: types.CallbackQuery):
    ...
    for coin in ALL_COINS:
        ...
        pars_price = (
            coin_price.split("CurrentPrice")[1]
            .replace(" ", "")
            .replace(")", "")
            .replace("(", "")
        )
...
@dp.callback_query_handler(
    lambda command: command.data and command.data.startswith("CTC_")
)
...
def register_monitoring_handlers(dp: Dispatcher):
    dp.register_message_handler(
        monitoring, lambda message: message.text == "📊 Monitoring"
    )
    dp.register_message_handler(
        tracked_coins, lambda message: message.text == "✍🏻 Tracked coins"
    )
    dp.register_message_handler(
        check_price, lambda message: message.text == "📈 Check price"
    )
    dp.register_message_handler(
        cp_add, lambda command: command.data and command.data.startswith("CP_")
    )
    dp.register_message_handler(
        ctc_add, lambda command: command.data and command.data.startswith("CTC_")
    )
