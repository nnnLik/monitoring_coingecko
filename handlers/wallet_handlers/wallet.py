...
async def wallet(message: types.Message):
    ...
    try:
        ...
        buttons = ["💳 Check the balance", "🛍 Buy / Sell", "🔙 Back"]
...
@dp.callback_query_handler(
    lambda command: command.data and command.data.startswith("CREATE")
)
...
@dp.callback_query_handler(
    lambda command: command.data and command.data.startswith("WC_")
)
...
def register_wallet_handlers(dp: Dispatcher):
    ...
    dp.register_message_handler(
        check_the_balance, lambda message: message.text == "💳 Check the balance"
    )
    dp.register_message_handler(
        buy_sell, lambda message: message.text == "🛍 Buy / Sell"
    )
    dp.register_message_handler(
        create_wallet,
        lambda command: command.data and command.data.startswith("CREATE"),
